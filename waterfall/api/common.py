...
api_common_opts = [
    ...
    cfg.StrOpt('osapi_workflow_base_URL',
               help='Base URL that will be presented to users in links '
                    'to the OpenStack Workflow API',
               deprecated_name='osapi_compute_link_prefix'),
    cfg.ListOpt('query_workflow_filters',
                default=['name', 'status', 'metadata',
                         'availability_zone',
                         'bootable'],
                help="Workflow filter options which "
                     "non-admin user could use to "
                     "query workflows. Default values "
                     "are: ['name', 'status', "
...
def validate_policy(context, action):
    try:
        waterfall.policy.enforce_action(context, action)
        return True
...
def get_sort_params(params, default_key='created_at', default_dir='desc'):
    ...
    :param params: webob.multidict of request parameters (from
                   waterfall.api.openstack.wsgi.Request.params)
...
def remove_version_from_href(href):
    ...
    Given: 'http://www.waterfall.com/v1.1/123'
    Returns: 'http://www.waterfall.com/123'

    Given: 'http://www.waterfall.com/v1.1'
    Returns: 'http://www.waterfall.com'
...
class ViewBuilder(object):
    ...
    def _get_next_link(self, request, identifier, collection_name):
        ...
        prefix = self._update_link_prefix(get_request_url(request),
                                          CONF.osapi_workflow_base_URL)
        url = os.path.join(prefix,
                           request.environ["waterfall.context"].project_id,
                           collection_name)
