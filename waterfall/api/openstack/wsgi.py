...
from waterfall.api.openstack import api_version_request as api_version
from waterfall.api.openstack import versioned_method
from waterfall import exception
from waterfall import i18n
from waterfall.i18n import _, _LE, _LI
from waterfall import utils
from waterfall.wsgi import common as wsgi
...
SUPPORTED_CONTENT_TYPES = (
    'application/json',
    'application/vnd.openstack.workflow+json',
    'application/xml',
    'application/vnd.openstack.workflow+xml',
)

_MEDIA_TYPE_MAP = {
    'application/vnd.openstack.workflow+json': 'json',
    'application/json': 'json',
    'application/vnd.openstack.workflow+xml': 'xml',
    'application/xml': 'xml',
...
class Request(webob.Request):
    ...
    def cache_resource(self, resource_to_cache, id_attribute='id', name=None):
        ...
        request, they can be cached using the name parameter. For example:
            ...
            Controller 1:
                request.cache_resource(db_workflows, 'workflows')
                request.cache_resource(db_workflow_types, 'types')
            Controller 2:
                db_workflows = request.cached_resource('workflows')
    ...
    def cache_db_workflows(self, workflows):
        # NOTE(mgagne) Cache it twice for backward compatibility reasons
        self.cache_db_items('workflows', workflows, 'id')
        self.cache_db_items(self.path, workflows, 'id')

    def cache_db_workflow(self, workflow):
        # NOTE(mgagne) Cache it twice for backward compatibility reasons
        self.cache_db_items('workflows', [workflow], 'id')
        self.cache_db_items(self.path, [workflow], 'id')

    def get_db_workflows(self):
        return (self.get_db_items('workflows') or
                self.get_db_items(self.path))

    def get_db_workflow(self, workflow_id):
        return (self.get_db_item('workflows', workflow_id) or
                self.get_db_item(self.path, workflow_id))

    def cache_db_workflow_types(self, workflow_types):
        self.cache_db_items('workflow_types', workflow_types, 'id')

    def cache_db_workflow_type(self, workflow_type):
        self.cache_db_items('workflow_types', [workflow_type], 'id')

    def get_db_workflow_types(self):
        return self.get_db_items('workflow_types')

    def get_db_workflow_type(self, workflow_type_id):
        return self.get_db_item('workflow_types', workflow_type_id)
    ...
    def best_match_content_type(self):
        ...
        if 'waterfall.best_content_type' not in self.environ:
            ...
            self.environ['waterfall.best_content_type'] = (content_type or
                                                        'application/json')

        return self.environ['waterfall.best_content_type']
    ...
    def set_api_version_request(self, url):
        ...
        if API_VERSION_REQUEST_HEADER in self.headers and 'v3' in url:
            ...
            for hdr in hdr_string_list:
                if WORKFLOW_SERVICE in hdr:
                    service, workflow_version = hdr.split()
                    break
...
class Resource(wsgi.Application):
    ...
    def _process_stack(self, request, action, action_args,
        ...
        context = request.environ.get('waterfall.context')
...
def _set_request_id_header(req, headers):
    context = req.environ.get('waterfall.context')
    if context:
