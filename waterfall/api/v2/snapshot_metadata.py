...
class Controller(wsgi.Controller):
    ...
    def _get_metadata(self, context, snapshot_id):
        try:
            snapshot = self.workflow_api.get_snapshot(context, snapshot_id)
            meta = self.workflow_api.get_snapshot_metadata(context, snapshot)
    ...
    def _update_snapshot_metadata(self, context,
        ...
        try:
            snapshot = self.workflow_api.get_snapshot(context, snapshot_id)
            return self.workflow_api.update_snapshot_metadata(context,
                                                            snapshot,
                                                            metadata,
                                                            delete)
        except exception.SnapshotNotFound:
    ...
    def delete(self, req, snapshot_id, id):
        ...
        try:
            snapshot = self.workflow_api.get_snapshot(context, snapshot_id)
            self.workflow_api.delete_snapshot_metadata(context, snapshot, id)
        except exception.SnapshotNotFound:
            msg = _('snapshot does not exist')
            raise exc.HTTPNotFound(explanation=msg)
