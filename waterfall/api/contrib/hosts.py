...
authorize = extensions.extension_authorizer('workflow', 'hosts')
...
class HostController(wsgi.Controller):
    ...
    def __init__(self):
        self.api = workflow_api.HostAPI()
        super(HostController, self).__init__()

    @wsgi.serializers(xml=HostIndexTemplate)
    def index(self, req):
        authorize(req.environ['waterfall.context'])
    ...
    def _set_enabled_status(self, req, host, enabled):
        """Sets the specified host's ability to accept new workflows."""
        context = req.environ['waterfall.context']
    ...
    def show(self, req, id):
        """Shows the workflow usage info given by hosts.
        ...
        :returns: expected to use HostShowTemplate.
            ex.::
                ...
                D: {'host': 'hostname','project': 'admin',
                    'workflow_count': 1, 'total_workflow_gb': 2048}
        """
        host = id
        context = req.environ['waterfall.context']
        ...
        try:
            host_ref = objects.Service.get_by_host_and_topic(
                context, host, CONF.workflow_topic)
        except exception.ServiceNotFound:
            raise webob.exc.HTTPNotFound(explanation=_("Host not found"))

        # Getting total available/used resource
        # TODO(jdg): Add summary info for Snapshots
        workflow_refs = db.workflow_get_all_by_host(context, host_ref.host)
        (count, sum) = db.workflow_data_get_for_host(context,
                                                   host_ref.host)

        snap_count_total = 0
        snap_sum_total = 0
        resources = [{'resource': {'host': host, 'project': '(total)',
                                   'workflow_count': str(count),
                                   'total_workflow_gb': str(sum),
                                   'snapshot_count': str(snap_count_total),
                                   'total_snapshot_gb': str(snap_sum_total)}}]

        project_ids = [v['project_id'] for v in workflow_refs]
        project_ids = list(set(project_ids))
        for project_id in project_ids:
            (count, sum) = db.workflow_data_get_for_project(context, project_id)
            (snap_count, snap_sum) = (
...
class Hosts(extensions.ExtensionDescriptor):
    ...
    name = "Hosts"
    alias = "os-hosts"
    namespace = "http://docs.openstack.org/workflow/ext/hosts/api/v1.1"
