...
from waterfall.api.views import backups as backup_views
from waterfall.api import xmlutil
from waterfall import backup as backupAPI
...
class CreateDeserializer(wsgi.MetadataXMLDeserializer):
    ...
    def _extract_backup(self, node):
        ...
        attributes = ['container', 'display_name',
                      'display_description', 'workflow_id',
                      'parent_id']
...
class RestoreDeserializer(wsgi.MetadataXMLDeserializer):
    ...
    def _extract_restore(self, node):
        ...
        if restore_node.getAttribute('workflow_id'):
            restore['workflow_id'] = restore_node.getAttribute('workflow_id')
        return restore
...
class BackupsController(wsgi.Controller):
    ...
    def _get_backup_filter_options():
        ...
        return ('name', 'status', 'workflow_id')
    ...
    # - whether requested workflow_id exists so we can return some errors
    #   immediately
    ...
    def create(self, req, body):
        ...
        try:
            workflow_id = backup['workflow_id']
        except KeyError:
        ...
        LOG.info(_LI("Creating backup of workflow %(workflow_id)s in container"
                     " %(container)s"),
                 {'workflow_id': workflow_id, 'container': container},
                 context=context)
    ...
    def restore(self, req, id, body):
        """Restore an existing backup to a workflow."""
        ...
        workflow_id = restore.get('workflow_id', None)
        name = restore.get('name', None)

        LOG.info(_LI("Restoring backup %(backup_id)s to workflow %(workflow_id)s"),
                 {'backup_id': id, 'workflow_id': workflow_id},
                 context=context)
        ...
        except exception.WorkflowLimitExceeded as error:
            raise exc.HTTPRequestEntityTooLarge(
                explanation=error.msg, headers={'Retry-After': '0'})
...
class Backups(extensions.ExtensionDescriptor):
    ...
    name = 'Backups'
    alias = 'backups'
    namespace = 'http://docs.openstack.org/workflow/ext/backups/api/v1'
