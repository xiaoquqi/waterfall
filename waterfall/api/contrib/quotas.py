...
from waterfall.db.sqlalchemy import api as sqlalchemy_api
...
authorize_update = extensions.extension_authorizer('workflow', 'quotas:update')
authorize_show = extensions.extension_authorizer('workflow', 'quotas:show')
authorize_delete = extensions.extension_authorizer('workflow', 'quotas:delete')
...
class QuotaSetsController(wsgi.Controller):
    ...
    def _validate_existing_resource(self, key, value, quota_values):
        ...
        if key == 'per_workflow_gigabytes' or value == -1:
            return
    ...
    def _update_nested_quota_allocated(self, ctxt, target_project,
        ...
        reservations = []
        # per_workflow_gigabytes doesn't make sense to nest
        if res == "per_workflow_gigabytes":
            return reservations
    ...
    def validate_setup_for_nested_quota_use(self, req):
        ...
        """
        ctxt = req.environ['waterfall.context']
        params = req.params
        try:
...
class Quotas(extensions.ExtensionDescriptor):
    ...
    name = "Quotas"
    alias = "os-quota-sets"
    namespace = "http://docs.openstack.org/workflow/ext/quotas-sets/api/v1.1"
