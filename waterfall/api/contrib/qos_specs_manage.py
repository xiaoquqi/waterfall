...
from waterfall.api.views import qos_specs as view_qos_specs
...
from waterfall.workflow import qos_specs


LOG = logging.getLogger(__name__)

authorize = extensions.extension_authorizer('workflow', 'qos_specs_manage')
...
class QoSSpecsController(wsgi.Controller):
    ...
    def associations(self, req, id):
        ...
        try:
            ...
            self._notify_qos_specs_error(context,
                                         ...
                                         notifier_err)
            raise webob.exc.HTTPNotFound(explanation=six.text_type(err))
        except exception.WaterfallException as err:
            notifier_err = dict(id=id, error_message=err)
            self._notify_qos_specs_error(context,
                                         'qos_specs.associations',
                                         notifier_err)
    ...
    def associate(self, req, id):
        """Associate a qos specs with a workflow type."""
        context = req.environ['waterfall.context']
        authorize(context)
        ...
        if not type_id:
            msg = _('Workflow Type id must not be None.')
        ...
        try:
            ...
            self._notify_qos_specs_error(context,
                                         ...
                                         notifier_err)
            raise webob.exc.HTTPNotFound(explanation=six.text_type(err))
        except exception.InvalidWorkflowType as err:
            notifier_err = dict(id=id, error_message=err)
            self._notify_qos_specs_error(context,
                                         'qos_specs.associate',
                                         notifier_err)
    ...
    def disassociate(self, req, id):
        """Disassociate a qos specs from a workflow type."""
        context = req.environ['waterfall.context']
        authorize(context)
    ...
    def disassociate_all(self, req, id):
        """Disassociate a qos specs from all workflow types."""
        context = req.environ['waterfall.context']
        authorize(context)
...
class Qos_specs_manage(extensions.ExtensionDescriptor):
    ...
    alias = "qos-specs"
    namespace = "http://docs.openstack.org/workflow/ext/qos-specs/api/v1"
