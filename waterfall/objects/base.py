...
"""Waterfall common internal object model"""
...
class WaterfallObjectVersionsHistory(dict):
    ...
    def __init__(self):
        super(WaterfallObjectVersionsHistory, self).__init__()
        ...
        self['liberty'] = {
            ...
            'SnapshotList': '1.0',
            'Workflow': '1.1',
            'WorkflowAttachment': '1.0',
            'WorkflowAttachmentList': '1.0',
            'WorkflowList': '1.1',
            'WorkflowType': '1.0',
            'WorkflowTypeList': '1.0',
        }
...
OBJ_VERSIONS = WaterfallObjectVersionsHistory()
...
OBJ_VERSIONS.add('1.0', {'Backup': '1.3', 'BackupImport': '1.3',
                         ...
                         'Workflow': '1.3', 'WorkflowTypeList': '1.1'})
...
class WaterfallObjectRegistry(base.VersionedObjectRegistry):
...
class WaterfallObject(base.VersionedObject):
    # NOTE(thangp): OBJ_PROJECT_NAMESPACE needs to be set so that nova,
    # waterfall, and other objects can exist on the same bus and be distinguished
    # from one another.
    OBJ_PROJECT_NAMESPACE = 'waterfall'

    # NOTE(thangp): As more objects are added to waterfall, each object should
    ...
    def waterfall_obj_get_changes(self):
    ...
    def conditional_update(self, values, expected_values=None, filters=(),
        ...
        """Compare-and-swap update.
           ...
           [~sql.exists().where(models.Workflow.id == models.Snapshot.workflow_id)]

           We can select values based on conditions using Case objects in the
           'values' argument. For example:
           has_snapshot_filter = sql.exists().where(
               models.Snapshot.workflow_id == models.Workflow.id)
           case_values = workflow.Case([(has_snapshot_filter, 'has-snapshot')],
                                     else_='no-snapshot')
           workflow.conditional_update({'status': case_values},
                                     {'status': 'available'}))
            ...
            workflow.conditional_update({'status': 'deleting',
                                       'previous_status': workflow.model.status},
        ...
        if save_all:
            changes = self.waterfall_obj_get_changes()
    ...
    def __contains__(self, name):
        ...
        # "'foo' in obj" tests.
        return name in self.obj_extra_fields or super(WaterfallObject,
                                                      self).__contains__(name)


class WaterfallObjectDictCompat(base.VersionedObjectDictCompat):
    ...
    def get(self, key, value=base._NotSpecifiedSentinel):
        ...
        if key not in self.obj_fields:
            ...
            # rely on it.
            LOG.debug('Waterfall object %(object_name)s has no '
...
class WaterfallPersistentObject(object):
...
class WaterfallComparableObject(base.ComparableVersionedObject):
    def __eq__(self, obj):
...
class WaterfallObjectSerializer(base.VersionedObjectSerializer):
    OBJ_BASE_CLASS = WaterfallObject

    def __init__(self, version_cap=None):
        super(WaterfallObjectSerializer, self).__init__()
