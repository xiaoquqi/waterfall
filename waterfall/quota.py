...
from waterfall import quota_utils


LOG = logging.getLogger(__name__)

quota_opts = [
    cfg.IntOpt('quota_workflows',
               default=10,
               help='Number of workflows allowed per project'),
    cfg.IntOpt('quota_snapshots',
               default=10,
               help='Number of workflow snapshots allowed per project'),
    ...
    cfg.IntOpt('quota_gigabytes',
               ...
               help='Total amount of storage, in gigabytes, allowed '
                    'for workflows and snapshots per project'),
    cfg.IntOpt('quota_backups',
               default=10,
               help='Number of workflow backups allowed per project'),
    ...
    cfg.StrOpt('quota_driver',
               default="waterfall.quota.DbQuotaDriver",
               help='Default driver to use for quota checks'),
    cfg.BoolOpt('use_default_quota_class',
                default=True,
                help='Enables or disables use of default quota class '
                     'with default quota.'),
    cfg.IntOpt('per_workflow_size_limit',
               default=-1,
               help='Max size allowed per workflow, in gigabytes'), ]
...
class NestedDbQuotaDriver(DbQuotaDriver):
    ...
    def _get_cur_project_allocated(self, ctxt, resource, project_tree):
        ...
        gets a workflow created, thus changing the in-use value and messing up
    ...
    def _reserve(self, context, resources, quotas, deltas, expire, project_id):
        ...
        if resources_failed_to_update:
            ...
            # We change OverQuota to OverWorkflowLimit in other places and expect
...
class BaseResource(object):
    ...
    def __init__(self, name, flag=None, parent_project_id=None):
        ...
        :param name: The name of the resource, i.e., "workflows".
...
class ReservableResource(BaseResource):
    ...
    def __init__(self, name, sync, flag=None):
        ...
        correspond to objects in the database, i.e., workflows, gigabytes,
...
class WorkflowTypeResource(ReservableResource):
    """ReservableResource for a specific workflow type."""

    def __init__(self, part_name, workflow_type):
        """Initializes a WorkflowTypeResource.

        :param part_name: The kind of resource, i.e., "workflows".
        :param workflow_type: The workflow type for this resource.
        """

        self.workflow_type_name = workflow_type['name']
        self.workflow_type_id = workflow_type['id']
        name = "%s_%s" % (part_name, self.workflow_type_name)
        super(WorkflowTypeResource, self).__init__(name, "_sync_%s" % part_name)
...
class QuotaEngine(object):
    ...
    def add_workflow_type_opts(self, context, opts, workflow_type_id):
        """Add workflow type resource options.

        Adds elements to the opts hash for workflow type quotas.
        If a resource is being reserved ('gigabytes', etc) and the workflow
        type is set up for its own quotas, these reservations are copied
        into keys for 'gigabytes_<workflow type name>', etc.

        :param context: The request context, for access checks.
        :param opts: The reservations options hash.
        :param workflow_type_id: The workflow type id for this reservation.
        """
        if not workflow_type_id:
            return

        # NOTE(jdg): set inactive to True in workflow_type_get, as we
        # may be operating on a workflow that was created with a type
        # that has since been deleted.
        workflow_type = db.workflow_type_get(context, workflow_type_id, True)

        for quota in ('workflows', 'gigabytes', 'snapshots'):
            if quota in opts:
                vtype_quota = "%s_%s" % (quota, workflow_type['name'])
...
class WorkflowTypeQuotaEngine(QuotaEngine):
    ...
    def resources(self):
        ...
        result = {}
        # Global quotas.
        argses = [('workflows', '_sync_workflows', 'quota_workflows'),
                  ('per_workflow_gigabytes', None, 'per_workflow_size_limit'),
        ...
        # Workflow type quotas.
        workflow_types = db.workflow_type_get_all(context.get_admin_context(),
                                              False)
        for workflow_type in workflow_types.values():
            for part_name in ('workflows', 'gigabytes', 'snapshots'):
                resource = WorkflowTypeResource(part_name, workflow_type)
                result[resource.name] = resource
        return result
    ...
    def update_quota_resource(self, context, old_type_name, new_type_name):
        ...
        quota_usages once the name of a workflow type is changed.

        :param context: The request context, for access checks.
        :param old_type_name: old name of workflow type.
        :param new_type_name: new name of workflow type.
        """
...
QUOTAS = WorkflowTypeQuotaEngine()
