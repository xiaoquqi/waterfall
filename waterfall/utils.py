...
from waterfall.i18n import _, _LE, _LW
...
synchronized = lockutils.synchronized_with_prefix('waterfall-')
...
def find_config(config_path):
    ...
    :raises: `waterfall.exception.ConfigNotFound`

    """
    possible_locations = [
        config_path,
        os.path.join(CONF.state_path, "etc", "waterfall", config_path),
        os.path.join(CONF.state_path, "etc", config_path),
        os.path.join(CONF.state_path, config_path),
        "/etc/waterfall/%s" % config_path,
    ]
...
def waterfalldir():
    import waterfall
    return os.path.abspath(waterfall.__file__).split('waterfall/__init__.py')[0]
...
def last_completed_audit_period(unit=None):
    ...
    """
    if not unit:
        unit = CONF.workflow_usage_audit_period
...
def safe_minidom_parse_string(xml_string):
    ...
    try:
        if six.PY3 and isinstance(xml_string, bytes):
            ...
            #
            # Bet that XML used in Waterfall is always encoded to UTF-8.
...
def monkey_patch():
    ...
    Example: 'waterfall.api.ec2.cloud:' \
     waterfall.openstack.common.notifier.api.notify_decorator'

    Parameters of the decorator is as follows.
    (See waterfall.openstack.common.notifier.api.notify_decorator)
...
def get_root_helper():
    return 'sudo waterfall-rootwrap %s' % CONF.rootwrap_config
...
def add_visible_admin_metadata(workflow):
    ...
    if workflow.get('workflow_admin_metadata'):
        if isinstance(workflow['workflow_admin_metadata'], dict):
            workflow_admin_metadata = workflow['workflow_admin_metadata']
            for key in workflow_admin_metadata:
                if key in _visible_admin_metadata_keys:
                    visible_admin_meta[key] = workflow_admin_metadata[key]
        else:
            for item in workflow['workflow_admin_metadata']:
                if item['key'] in _visible_admin_metadata_keys:
                    visible_admin_meta[item['key']] = item['value']
    # avoid circular ref when workflow is a Workflow instance
    elif (workflow.get('admin_metadata') and
            isinstance(workflow.get('admin_metadata'), dict)):
        for key in _visible_admin_metadata_keys:
            if key in workflow['admin_metadata'].keys():
                visible_admin_meta[key] = workflow['admin_metadata'][key]

    if not visible_admin_meta:
        return

    # NOTE(zhiyan): update visible administration metadata to
    # workflow metadata, administration metadata will rewrite existing key.
    if workflow.get('workflow_metadata'):
        orig_meta = list(workflow.get('workflow_metadata'))
        ...
        workflow['workflow_metadata'] = orig_meta
    # avoid circular ref when vol is a Workflow instance
    elif (workflow.get('metadata') and
            isinstance(workflow.get('metadata'), dict)):
        workflow['metadata'].update(visible_admin_meta)
    else:
        workflow['metadata'] = visible_admin_meta
