...
class Controller(wsgi.Controller):
    ...
    def _update_workflow_metadata(self, context,
        ...
        try:
            workflow = self.workflow_api.get(context, workflow_id)
            return self.workflow_api.update_workflow_metadata(context,
                                                          workflow,
                                                          metadata,
                                                          delete)
        except exception.WorkflowNotFound:
    ...
    def delete(self, req, workflow_id, id):
        ...
        metadata = self._get_metadata(context, workflow_id)
        ...
        try:
            workflow = self.workflow_api.get(context, workflow_id)
            self.workflow_api.delete_workflow_metadata(context, workflow, id)
        except exception.WorkflowNotFound:
            msg = _('workflow does not exist')
            raise exc.HTTPNotFound(explanation=msg)
